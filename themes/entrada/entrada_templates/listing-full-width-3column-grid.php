<?php 
/*
Template Name: Listing Full Width (three column grid)
*/
get_header();
global $wp_customize;
while ( have_posts() ) : the_post();
	...
	<?php
		...
		if( 'grid' == $view_mode ) {
			get_template_part( 'template-parts/grid', 'threecolumn' );
			echo '<input type="hidden" class="view_mode" value="'.esc_attr( 'grid' ).'">';
			echo '<input type="hidden" class="ajax_action" value="'.esc_attr( 'threecol' ).'">';
		}
		else{
