<?php 
/*
Template Name: Listing Full Width (two column grid)
*/
get_header();
global $wp_customize;
...
get_template_part( 'template-parts/banner', 'section' ); ?>
	...
	<?php
		...
		if( 'grid' == $view_mode ) {
			get_template_part( 'template-parts/grid', 'twocolumn' );
			echo '<input type="hidden" class="view_mode" value="'.esc_attr( 'grid' ).'">';
			echo '<input type="hidden" class="ajax_action" value="'.esc_attr( 'twocol' ).'">';
		}
		else{
