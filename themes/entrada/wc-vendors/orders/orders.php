		...
		<thead>
			<tr>
				<?php foreach ($headers as $header) : ?>
					<th class="<?php echo sanitize_title($header); ?>"><?php echo sprintf(__('%s', 'entrada'), $header); ?></th>
				<?php endforeach; ?>
			</tr>
		</thead>
		<tbody>
				...
				<tr>
					...
					<?php
						...
						<?php if ($detail_key == $first_index) :
									$cnt_chk = ($count == 1) ? 3 : ($count + 3);
									?>

							<td class="<?php echo esc_attr($detail_key);  ?>" rowspan="<?php echo esc_attr($cnt_chk); ?>"><?php
																																		echo sprintf(__('%s', 'entrada'), $detail);
																																		?></td>

						<?php else : ?>

							<td class="<?php echo esc_attr($detail_key); ?>"><?php
																							echo sprintf(__('%s', 'entrada'), $detail);
																							?></td>
					...
					<td colspan="100%">
						...
						<?php
							...
							<a href="#" class="order-comments-link">
								<p>
									<?php printf(__('Comments (%s)', 'entrada'), count($comments)); ?>
								</p>
							</a>
